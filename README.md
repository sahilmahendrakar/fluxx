...
Task tools are named `flux__list_tasks`, `flux__create_task`, `flux__start_task`, `flux__update_task`, `flux__delete_task`, and `flux__get_project_info`. They operate on the **currently open** local project’s board in the app. `flux__create_task` and `flux__update_task` accept an optional `labels` array for feature tags; values are normalized (trimmed, no empties, case-insensitive unique). `flux__delete_task` requires `confirm: true` in the tool arguments after the user explicitly asked to remove that task.

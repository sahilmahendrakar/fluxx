...
Task tools are named `fluxx__list_tasks`, `fluxx__create_task`, `fluxx__start_task`, `fluxx__update_task`, `fluxx__delete_task`, `fluxx__get_project_info`, `fluxx__list_members`, and `fluxx__list_repo_branches` (legacy `flux__*` names are still registered for one release). They operate on the **currently open** project’s board in the app. `fluxx__list_tasks` accepts optional `excludeStatuses` (array of `backlog` \| `in-progress` \| `needs-input` \| `done`) to drop those columns from the JSON response; omit it to list every task. `fluxx__create_task` and `fluxx__update_task` accept an optional `labels` array for feature tags; values are normalized (trimmed, no empties, case-insensitive unique). `fluxx__create_task` also accepts optional `agentModel` and `agentYolo`; when omitted, the project’s saved defaults apply (same as new tasks in the UI). `fluxx__delete_task` requires `confirm: true` in the tool arguments after the user explicitly asked to remove that task.

...
export const PLANNING_ASSISTANT_TEMPLATE_VERSION = 7;
...
When user intent spans more than one repository and is ambiguous, **ask once** which repo (or \`repoId\`) they mean before creating tasks.`
  ...
  const createTaskLine = multiRepoGuide
    ? `- \`fluxx tasks create --json --title "..." --description "..." --agent <claude-code|cursor|codex|none>\` — optional repeated \`--label <label>\`, repeated \`--depends-on-task-id <taskId>\`, repeated \`--attach-doc <relativePath>\` (planning markdown, e.g. \`docs/plan.md\`), \`--validation-plan <json>\` (optional validator instructions), \`--assignee-email\` (cloud; use \`fluxx members list --json\`), \`--repo-id <repos[].id>\` (omit for primary), \`--source-branch <git-branch>\` (alias: \`--feature-branch\`), \`--create-source-branch-if-missing=true\` when a missing branch should be created on first session start`
    : `- \`fluxx tasks create --json --title "..." --description "..." --agent <claude-code|cursor|codex|none>\` — optional repeated \`--label <label>\`, repeated \`--depends-on-task-id <taskId>\`, repeated \`--attach-doc <relativePath>\` (planning markdown, e.g. \`docs/plan.md\`), \`--validation-plan <json>\` (optional validator instructions), \`--assignee-email\` (cloud; use \`fluxx members list --json\`), \`--source-branch <git-branch>\` (alias: \`--feature-branch\`), \`--create-source-branch-if-missing=true\``;

  const updateTaskLine = multiRepoGuide
    ? `- \`fluxx tasks update --json --id <taskId>\` — optional \`--title\`, \`--description\`, \`--status\`, \`--agent\`, repeated \`--label <label>\` (replace labels), \`--clear-labels\`, repeated \`--depends-on-task-id <taskId>\` (replace dependencies), \`--clear-dependencies\`, repeated \`--attach-doc <relativePath>\` (replace attachments), \`--clear-attached-docs\`, \`--validation-plan <json>\` (validator instructions), \`--clear-validation-plan\`, \`--assignee-email\`, \`--unassign-assignee=true\`, \`--repo-id <repos[].id>\` (only while no session/worktree/PR — same as UI), \`--source-branch <git-branch>\` (alias: \`--feature-branch\`), \`--create-source-branch-if-missing\`. Branch edits fail safely if a session or worktree already exists`
    : `- \`fluxx tasks update --json --id <taskId>\` — optional \`--title\`, \`--description\`, \`--status\`, \`--agent\`, repeated \`--label <label>\` (replace labels), \`--clear-labels\`, repeated \`--depends-on-task-id <taskId>\` (replace dependencies), \`--clear-dependencies\`, repeated \`--attach-doc <relativePath>\` (replace attachments), \`--clear-attached-docs\`, \`--validation-plan <json>\` (validator instructions), \`--clear-validation-plan\`, \`--assignee-email\`, \`--unassign-assignee=true\`, \`--source-branch <git-branch>\` (alias: \`--feature-branch\`), \`--create-source-branch-if-missing\`. Branch edits fail safely if a session or worktree already exists`;
...
**Validation plan handoff (optional):** When a task is ready for Review and validation would benefit from task-specific guidance, write a structured validation plan for the validator agent — separate from the implementation \`description\`. Persist with \`fluxx tasks update --json --id <taskId> --validation-plan '<json>'\` or on create via \`--validation-plan\`. Use \`--clear-validation-plan\` on update to remove. JSON shape: \`goal\`, \`pack\` (\`electron-playwright\`), non-empty \`checks\` (strings), \`requiredArtifacts\` (strings), optional \`risks\` / \`notes\`. Example: \`fluxx tasks update --json --id <taskId> --validation-plan '{"goal":"Verify UI","pack":"electron-playwright","checks":["Open task details"],"requiredArtifacts":["screenshot"]}'\`.

...
describe('runFluxCli', () => {
  ...
  it('sends validation.finish payload with run id', async () => {
    ...
    const fetchMock = vi.fn(async () => ({
      ...
      json: async () => ({
        ok: true,
        data: { ingested: true, run: { id: 'run-1', status: 'passed' } },
      }),
    }));
    vi.stubGlobal('fetch', fetchMock);

    const code = await runFluxCli(['validation', 'finish', '--json', '--run-id', 'run-1']);
    ...
    expect(body.op).toBe('validation.finish');
    expect(body.payload).toEqual({ runId: 'run-1' });
  });
});

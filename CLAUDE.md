...
Planning workspaces created by Flux also ship `planning/AGENTS.md` (and `planning/CLAUDE.md`) with the same tool list; keep those aligned when editing assistant guidance.

## Cloud planning-docs sync (maintainer model)

**Goal:** Cloud projects mirror team planning markdown from Firestore into the same on-disk `planning/` tree the planning agent uses, with optimistic concurrency and recoverable conflicts.

**Authoritative source:** For cloud workspaces, Firestore collection `projects/{projectId}/planningDocs/{docId}` is the shared source of truth once migration completes. The desktop app writes mirrored `.md` under the worktree `planning/` folder. Local-only projects never touch Firestore; they keep using disk as today.
...
1. **Two teammates:** User A edits planning markdown (agent or editor); user B sees updates in the in-app Docs UI and on disk under `planning/` after sync.
...
5. **Scope:** Only planning markdown under `planning/` participates in sync; other files under `.flux/<project>/` are not uploaded as planning docs (push listing skips sync internals; non-`.md` is ignored by list providers).
